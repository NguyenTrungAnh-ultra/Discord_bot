...
from core.utils.validation import validate_symbol
from core.utils.logger import get_logger
from core.utils.user_agent import get_headers
from core.utils.transform import replace_in_column_names, flatten_hierarchical_index
...
if __name__ == "__main__":
    try:
        # 1. Khởi tạo đối tượng Finance cho mã chứng khoán (ví dụ: VCI)
        # period có thể là 'quarter' (quý) hoặc 'year' (năm)
        vci_finance = Finance(symbol='VCI', period='quarter', show_log=True)

        print("=== ĐANG TẢI DỮ LIỆU... ===\n")

        # 2. Lấy Báo cáo Kết quả kinh doanh (Income Statement)
        df_kqkd = vci_finance._process_report(
            report_key='Chỉ tiêu kết quả kinh doanh', 
            lang='vi',       # Chọn ngôn ngữ hiển thị
            dropna=True      # Loại bỏ các dòng toàn số 0 cho gọn
        )
        
        print("--- 1. CHỈ TIÊU KẾT QUẢ KINH DOANH ---")
        # Sử dụng to_string() để in toàn bộ dataframe mà không bị cắt dòng
        print(df_kqkd.head(10).to_string()) 
        print("\n")

        # 3. Lấy Báo cáo Cân đối kế toán (Balance Sheet)
        df_cdkt = vci_finance._process_report(
            report_key='Chỉ tiêu cân đối kế toán',
            lang='vi'
        )
        
        print("--- 2. CHỈ TIÊU CÂN ĐỐI KẾ TOÁN ---")
        print(df_cdkt.iloc[:, :5].head(10).to_string()) # In thử 5 cột đầu, 10 dòng đầu
        print("\n")

        # 4. Lấy Báo cáo Lưu chuyển tiền tệ (Cash Flow)
        df_lctt = vci_finance._process_report(
            report_key='Chỉ tiêu lưu chuyển tiền tệ',
            lang='vi'
        )
        
        print("--- 3. CHỈ TIÊU LƯU CHUYỂN TIỀN TỆ ---")
        print(df_lctt.tail(5).to_string()) # In 5 dòng cuối
