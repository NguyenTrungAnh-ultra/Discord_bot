"""
VietCap Report Scraper
Scrapes financial reports from VietCap website using Playwright
Website blocks HTTP requests (403) and may require authentication
"""
...
class Config:
    """Configuration for VietCap scraper"""
...
class VietCapReportScraper(VietCapBaseScraper):
    """Main scraper for VietCap reports"""
    
    def __init__(self, headless: bool = True, slow_mo: int = 0):
        super().__init__(headless, slow_mo)
        self.base_dir = r".\temp\reports\VietCap"
    ...
    def _get_csv_path(self, report_type: str) -> str:
        ...
        return os.path.join(report_dir, "vietcap_reports.csv")
    ...
    async def _wait_for_reports_loaded(self, page: Page) -> bool:
        ...
        try:
            ...
            print("⚠️ No report containers found")
            return False
        ...
        try:
            ...
            for link in links:
                try:
                    ...
                    # Filter for report-like links
                    if not any(keyword in href.lower() for keyword in ['report', 'bao-cao', 'analysis', 'research']):
                        continue
    ...
    async def scan_by_type(self, report_type: str, max_pages: int = Config.max_pages_per_type) -> Dict:
        ...
        print(f"\n{'='*60}")
        print(f"🔍 Scanning VietCap: {report_type}")
        print(f"{'='*60}\n")
        ...
        try:
            # Get type URL
            type_url = Config.report_types.get(report_type, Config.report_types['iq_report'])
            ...
            # Wait for reports
            await self._wait_for_reports_loaded(page)
...
async def scan_all(headless: bool = True) -> Dict:
    ...
    async with VietCapReportScraper(headless=headless) as scraper:
        results = {}
...
if __name__ == "__main__":
    print("🚀 Testing VietCap Scraper\n")
    
    # Test with visible browser
    result = asyncio.run(scan_type_async("iq_report", headless=False))
